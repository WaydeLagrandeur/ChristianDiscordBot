...
client.on('message', message => {   
    let badWords = ['fuck', 'shit', 'cunt', 'cock', 'dick', 'bitch', 'bastard', 'damn', 'dammit', 'ass', 'hell', 'pussy', 'whore', 'slut', 'piss', 'tit', 'cum', 'fag', 'linux'];
    let goodWords = ['frick', 'crap', 'good fellow', 'penis', 'penis', 'female dog', 'child of unwed parents', 'darn', 'darn it', 'bum', 'heck', 'vagina', 'promiscuous woman', 'promiscuous woman', 'pee', 'breast', 'semen', 'bundle of twigs', 'windows'];
    let bBadWords = false;
    ...
    if(bBadWords) {
        ...
        UpdateCountDB(message.author.username, message.channel.id, 'sin');
    }
});
...
async function UpdateCountDB(username, channel, count) {
    
    const user = await User.findOne({ where: { name: username, serverID: channel }});

    if(user) {
        if(count === 'sin') {
            user.increment('sinCount');
        }
        if(count === 'praise') {
        ...
        if(count === 'sin') {
            newUser.increment('sinCount');
        }
        if(count === 'praise') {
            newUser.increment('praiseCount');
        }
        if(count === 'verse') {
            newUser.increment('versesRead');
        }
    }
}
